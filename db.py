...
from config import ADMIN_ID, AGE_RANGE, DB_PATH, DAILY_LIKE_LIMIT, MIN_AGE
...
def _candidate_base_sql(search_city: str) -> tuple[str, list]:
    city_filter_sql = ""
    params: list = []
...
async def diagnose_candidates(user_id: int) -> dict:
    ...
    user_age = user["age"]
    min_age = max(MIN_AGE, user_age - AGE_RANGE)
    max_age = user_age + AGE_RANGE
    search_city = user_search_city(user).strip()

    base_sql, extra_params = _candidate_base_sql(search_city)
...
async def get_candidates(
    user_id: int, limit: int = 1, *, _allow_view_reset: bool = True
) -> list[dict]:
    user = await get_user(user_id)
    if not user:
        return []
    ...
    if not result:
        ...
        if diag.get("all_viewed") and _allow_view_reset:
            cleared = await clear_views(user_id)
            logger.info(
                "Reset %s views for user %s — new profiles available",
                cleared,
                user_id,
            )
            return await get_candidates(
                user_id, limit=limit, _allow_view_reset=False
            )
